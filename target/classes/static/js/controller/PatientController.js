angular.module("myApp").controller('PatientCtrl', ['$scope', '$http', 'PatientService', function ($scope, $http, PatientService) {
    $scope.patientList = PatientService.getPatientList();
    $scope.newPatient={};
    $scope.intializeUploadImage = function () {
        ...
        if ($scope.userMedia()) {
            ...
            $scope.addNewPatient = function (form) {
            	$http.post(form)
                console.log($scope);
                console.log(form);
                debugger;
